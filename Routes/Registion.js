...
const assosiateuser = require("../DBSchema/assosiateuser")
...
router.route("/createuser").post(expressAsyncHandler( async (req, res)=>{
    ...
    const existing = await assosiateuser.findOne({email});
    if(existing){
        res.status(400)
        throw new Error("email already Existed.")
    }
...
router.route("/user/:id").get(authorize([roles.SUPER_ADMIN, roles.ADMIN, roles.MANAGER]),expressAsyncHandler( async (req, res)=>{
    try {
        const data = await assosiateuser.findById(req.params.id);
...
router.route("/update/:id").put(authorize([roles.SUPER_ADMIN]), expressAsyncHandler(async(req, res)=>{
    ...
    // }
    try {
        const updateduser = await assosiateuser.findByIdAndUpdate(
            req.params.id,
            req.body,
            {new: true}
        );
